...
#include <iostream>
int next_id = 0; 
Track::Track(KAL_MEAN & mean, KAL_COVA & covariance, int track_id, int n_init, int max_age, const FEATURE & feature,DETECTION_ROW det)
{
    this->cur_det = det;
    this->mean = mean;
...
Track::Track(KAL_MEAN & mean, KAL_COVA & covariance, int track_id, int n_init, int max_age, const FEATURE & feature, int cls, float conf,DETECTION_ROW det)
{
    this->cur_det = det;
    this->mean = mean;
    this->covariance = covariance;
...
DETECTION_ROW Track::get_det() {
    DETECTBOX tlwh = this->to_tlwh();
    // tlwh: [top, left, width, height]
    cur_det.track_rect = cv::Rect_<float>(
        tlwh(0),        // x (left)
        tlwh(1),        // y (top)
        tlwh(2),        // width
        tlwh(3)         // height
