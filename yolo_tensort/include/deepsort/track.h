...
public:
    Track(KAL_MEAN& mean, KAL_COVA& covariance, int track_id,
          int n_init, int max_age, const FEATURE& feature,DETECTION_ROW det);
    Track(KAL_MEAN& mean, KAL_COVA& covariance, int track_id,
          int n_init, int max_age, const FEATURE& feature, int cls, float conf,DETECTION_ROW det);
    ...
    void mark_missed();
    DETECTION_ROW get_det();
    bool is_confirmed();
    bool is_deleted();
    bool is_tentative();
    DETECTBOX to_tlwh();
    DETECTION_ROW cur_det;
