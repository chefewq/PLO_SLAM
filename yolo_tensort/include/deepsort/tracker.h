...
public:
    ...
    tracker(/*NearNeighborDisMetric* metric,*/
            ...
            int max_age = 400, int n_init = 20);
    void predict();
    void update(const DETECTIONS &detections);
    void update(const DETECTIONSV2 &detectionsv2);
    void update(const DETECTIONSV2 &detectionsv2, std::map<int, cv::Rect> &obj_proj);
...
private:
    ...
    void _match(const DETECTIONS &detections, TRACHER_MATCHD &res, std::map<int, cv::Rect> &obj_proj);
