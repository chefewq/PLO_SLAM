...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mbShutDown(false),
    mnSaveMapCount(0), mbSaveMap(true), mbPaused(false)
{
    ...
    if(mSensor==RGBD)
    {    mpObjectTrack = new ObjectTrack(settings_->yoloEigen(),settings_->sortEigen(),mpTracker->getMatK()); 
    mpObjectTrack->setAtlas(mpAtlas);
    mptObjectTracking = new thread (&ORB_SLAM3::ObjectTrack::run,mpObjectTrack);
    }

     mpLocalMapper->SetObjectTrack(mpObjectTrack);
    ...
    mpTracker->SetObjectTrack(mpObjectTrack);
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, std::string filename)
{
...
void System::Shutdown()
{
    ...
    mpPointCloudMapper->RequestFinsh();
    if(mpViewer)
    {
    ...
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA()||!mpPointCloudMapper->isFinshd())
    {
        if(!mpLocalMapper->isFinished())
            cout << "mpLocalMapper is not finished" << endl;
        if(!mpLoopCloser->isFinished())
            cout << "mpLoopCloser is not finished" << endl;
        if(!mpPointCloudMapper)
        {
             cout << "mpPointCloudMapper is not finished" << endl;
        }
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    vector<KeyFramePtr> vpKFs = mpAtlas->GetAllKeyFrames();
    ...
    list<ORB_SLAM3::KeyFramePtr>::iterator lRit = mpTracker->mlpReferences.begin();
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    vector<KeyFramePtr> vpKFs = pBiggerMap->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
...
void System::EnableLoopClosing() 
{
    ...
    mpAtlas->mnEnableLoopClosing = true;
}

void System::DisableLoopClosing() 
{
    unique_lock< mutex > lock(mpAtlas->mMutexLoopClosing);
    mpAtlas->mnEnableLoopClosing = false;
}
...
void System::SaveCurrentMap() 
{
    ...
    SaveCurrentMap( map_name.str() );
}

void System::SaveCurrentMap(const string &filename) 
{
    if( filename.empty() ) 
    {
    ...
    std::cout << "Saving sparse map to file: " << filename << std::flush;
    boost::archive::binary_oarchive oa(out, boost::archive::no_header);
    oa << mpAtlas->GetCurrentMap();
    oa << mpKeyFrameDatabase;
    out.close();
    std::cout << " ...done" << std::endl << std::flush;    
    
    mpAtlas->GetCurrentMap()->printStatistics();
}

bool System::LoadCurrentMap(const string &filename) 
{
    ...
    cout << "Loading Map file: " << filename << std::flush;
    boost::archive::binary_iarchive ia(in, boost::archive::no_header);
    Map* map;
    ia >> map;
    mpAtlas->AddNewMap(map);
    ia >> mpKeyFrameDatabase;
    mpKeyFrameDatabase->SetORBVocabulary(mpVocabulary);
    cout << " ...done" << std::endl;
    cout << "Map Reconstructing" << flush;
    vector< ORB_SLAM3::KeyFramePtr> vpKFS = mpAtlas->GetAllKeyFrames();
...
void System::SaveAtlas(int type) 
{
    ...
    std::string tmpFilename( Utils::getFileNameWithouExtension(mStrMapfile) );
    if( tmpFilename.empty() ) tmpFilename = "sparse_atlas";
    
    std::stringstream map_name;
    map_name << tmpFilename << "_out_" << mnSaveMapCount++ << ".atlas";   
    
    SaveAtlas(map_name.str(), type);
}
...
bool System::LoadAtlas(const string &filename, int type) 
{
    ...
    for (auto pMap: vMaps)
    {
        vector<ORB_SLAM3::KeyFramePtr> vpKFS = pMap->GetAllKeyFrames();
        for (auto it : vpKFS) 
        {
            it->SetORBVocabulary(mpVocabulary);
