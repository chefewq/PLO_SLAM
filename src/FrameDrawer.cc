...
static const std::vector<cv::Scalar> kColorLinesOctave {
    cv::Scalar(255, 0, 0), // level 0
    cv::Scalar(255, 0, 0), // level 1
    cv::Scalar(255, 0, 0), // level 2
    cv::Scalar(255, 0, 0), // level 3
    cv::Scalar(255, 0, 0), // level 4
    cv::Scalar(255, 0, 0), // level 5
    cv::Scalar(255, 0, 0), // level 6
    cv::Scalar(255, 0, 0), // level 7
    cv::Scalar(255, 0, 0), // level 8
    cv::Scalar(255, 0, 0), // level 9
    cv::Scalar(255, 0, 0), // level 10
    cv::Scalar(255, 0, 0), // level 11
};
    ...
    //Copy variables within scoped mutex
    {
        ...
        else if(mState==Tracking::OK)
        {
            ...
            if(mbLineTracking)
            {
                ...
                // cout<<vCurrentKeyLines.size()<<endl;
                vbLineVO  = mvbLineVO;
...
void FrameDrawer::Update(Tracking *pTracker)
{
        ...
        int nMPs = mpAtlas->MapPointsInMap();
        int nMLs = mpAtlas->MapLinesInMap();
        std::ofstream fout_orb("orb_match.txt", std::ios::app);
fout_orb << mCurrentFrame.mnId << " " << mnTracked<< std::endl;
fout_orb.close();

std::ofstream fout_line("line_match.txt", std::ios::app);
fout_line << mCurrentFrame.mnId << " " << mnLinesTracked << std::endl;
fout_line.close();
    unique_lock<mutex> lock(mMutex);
