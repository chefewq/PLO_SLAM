...
#define USE_NEW_HISTOGRAM_FACTOR 1 
    ...
    // used by Tracking::SearchLocalPoints()
    int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPointPtr> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
    {
        ...
        //std::cout << "ORBmatcher::MatchByProjection() F-Map - matched " << nmatches << " points" << ", bFactor: " << int(bFactor) << ", th: " << th << std::endl;
    ...
    bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,const cv::KeyPoint &kp2,const cv::Mat &F12,const KeyFramePtr& pKF2, const bool b1)
    {
    ...
    int ORBmatcher::SearchByBoW(KeyFramePtr&  pKF,Frame &F, vector<MapPointPtr> &vpMapPointMatches)
    {
        ...
        vpMapPointMatches = vector<MapPointPtr>(F.N,static_cast<MapPointPtr>(NULL));
    ...
    // used in Tracking::Relocalization() and LoopClosing::ComputeSim3()
    ...
    int ORBmatcher::SearchBySim3(KeyFramePtr& pKF1, KeyFramePtr& pKF2, std::vector<MapPointPtr> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
        ...
        if(mbCheckOrientation)
        {
            ...
            for(int i=0; i<HISTO_LENGTH; i++)
            {
                if(i!=ind1 && i!=ind2 && i!=ind3)
                {
                    for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
                    {
                        CurrentFrame.mvpMapPoints[rotHist[i][j]]=static_cast<MapPointPtr>(NULL);
                        nmatches--;
                    }
                }
            }

        //std::cout << "ORBmatcher::MatchByProjection() F-F - matched " << nmatches << " points" << std::endl;
            return nmatches;
        }
    ...
    // used in LoopClosing::ComputeSim3() and Tracking::Relocalization()
