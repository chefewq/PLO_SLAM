...
void Tracking::SetObjectTrack(ObjectTrack *pObjectTrack)
{
    mpObjectTrack=pObjectTrack;
}
...
cv::Mat CorrectDepth(const cv::Mat &depthRaw)
{
    cv::Mat depthFloat;
    if(depthRaw.type() != CV_32F)
        depthRaw.convertTo(depthFloat, CV_32F, 5000);
    else
        depthFloat = depthRaw.clone();
    ...
    for(int r=0;r<depthFloat.rows;r++)
    {
        float* ptr = depthFloat.ptr<float>(r);
        for(int c=0;c<depthFloat.cols;c++)
        {
            float d = ptr[c];
            float d_metric = d0 + d1*d + d2*d*d + d3*d*d*d + d4*d*d*d*d;
            if(d_metric <= 0) d_metric = 0.01f; // 避免 0 或负数
            ptr[c] = d_metric;
        }
    }
    return depthFloat;
...
{    
    ...
    mCurrentFrame.mNameFile = filename;
    mCurrentFrame.mnDataset = mnNumDataset;
    mpObjectTrack->deepth =mImDepth;
#ifdef REGISTER_TIMES
    vdORBExtract_ms.push_back(mCurrentFrame.mTimeORB_Ext);
#endif
    while (true) {
    usleep(1);
    if(mpObjectTrack->TrackFinsh())
    break;
    }
    mpObjectTrack->Show();
    mpObjectTrack->getDynaMask();
    mCurrentFrame.dynaMask = mpObjectTrack->dynaMask; 
    //mpObjectTrack->updateObjectMap(&mLastFrame);
    mCurrentFrame.PointFilter(mImGray,mpObjectTrack->dynamicObjects,mImDepth);

std::ofstream fout_orb("orb_extract.txt", std::ios::app);
fout_orb << mCurrentFrame.mnId << " " << mCurrentFrame.mvKeys.size() << std::endl;
fout_orb.close();

std::ofstream fout_line("line_extract.txt", std::ios::app);
fout_line << mCurrentFrame.mnId << " " << mCurrentFrame.mvKeyLines.size() << std::endl;
fout_line.close();

    Track();

    KeyFramePtr kf = nullptr;
    if(mCurrentFrame.mnId == mnLastKeyFrameId)
    kf= mpLastKeyFrame;

    mpObjectTrack->updateTrack(mCurrentFrame,kf);
    mpObjectTrack->track(mpAtlas->GetCurrentMap(),mCurrentFrame);
    mpObjectTrack->trackDynamicObject(mpAtlas->GetCurrentMap(),mCurrentFrame);
    // TOCKTRACK("Track");
...
bool Tracking::TrackLocalMap()
{
    ...
    if(mbLineTrackerOn)
        SearchLocalLines();
cv::Mat depth_show;
mImDepth.convertTo(depth_show, CV_8U,10); 
...
#if 1    
    const int maxNumOfKeyFramesInLocalMapperQueue = ( mbLineTrackerOn) ? 5 : 3; // 3 was originally used without lines and volumetric mapping; 
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->mvDynamicRect = mCurrentFrame.mvDynamicRect;
    mpPointCloudMapper->InsertKeyFrame(pKF, mImColor, mImDepth,mpObjectTrack->Objects); 
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFramePtr pCurrentKeyFrame)
{
    ...
    unsigned int index = mnFirstFrameId;
    list<ORB_SLAM3::KeyFramePtr>::iterator lRit = mlpReferences.begin();
