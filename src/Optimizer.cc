...
#define USE_BA_VARIABLE_SIZE_SOLVER  (USE_LINES) // N.B.: this is compulsory when you use lines or objects 
...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    BundleAdjustment(vpKFs, vpMPoints, vpMLines,  nIterations, pbStopFlag, nLoopKF, bRobust);
}


void Optimizer::BundleAdjustment(const vector<KeyFramePtr> &vpKFs, const vector<MapPointPtr> &vpMPoints, 
                                 const vector<MapLinePtr> &vpMLines,
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    vector<ORB_SLAM3::EdgeSE3ProjectLineToBody*> vpLineEdgesBody;
        ...
        for(map<KeyFramePtr,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mpCamera2)
            {
                ...
                if(rightIndex != -1 && rightIndex < pKF->mvKeyLinesRightUn.size())
                {
                    ...
                    ORB_SLAM3::EdgeSE3ProjectLineToBody* e = new ORB_SLAM3::EdgeSE3ProjectLineToBody();
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    vector<ORB_SLAM3::EdgeSE3ProjectLineOnlyPoseToBody*> vpEdgesLineMonoRight;
    ...
    for(int i=0; i<Nlines; i++)
    {
        ...
        if(pML)
        {
            ...
            { // Right image SLAM 
                ...
                ORB_SLAM3::EdgeSE3ProjectLineOnlyPoseToBody* e = new ORB_SLAM3::EdgeSE3ProjectLineOnlyPoseToBody();
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        {
            ORB_SLAM3::EdgeSE3ProjectLineOnlyPoseToBody* e = vpEdgesLineMonoRight[i];
...
void Optimizer::LocalBundleAdjustment(KeyFramePtr pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_MLs, int& num_edges)
{
    ...
    // body lines
    vector<ORB_SLAM3::EdgeSE3ProjectLineToBody*> vpEdgesLineBody;
...
// -----------------------------------------------------------------------------
    ...
    {
        ORB_SLAM3::EdgeSE3ProjectLineToBody* e = vpEdgesLineBody[i];
...
void Optimizer::OptimizeEssentialGraph(KeyFramePtr pCurKF, vector<KeyFramePtr> &vpFixedKFs, vector<KeyFramePtr> &vpFixedCorrectedKFs,
                                       ...
                                       vector<MapLinePtr> &vpNonCorrectedMLs)
{
