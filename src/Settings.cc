...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false), bNeedToRectifyFishEye_(false) {
        ...
        readTensortEigen(fSettings);
         cout << "\t-Loaded TensortEigen parameters" << endl;
    ...
    void Settings::readTensortEigen(cv::FileStorage &fSettings){
        bool found;
        yoloEigenPath_ = readParameter<std::string>(fSettings,"Yolo.Eingen",found,true);
        sortEigenPath_ = readParameter<std::string>(fSettings, "Sort.Eingen", found,true);
    }
