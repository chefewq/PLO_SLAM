...
void LoopClosing::CorrectLoop()
{
    ...
    // Fuse duplications.
    SearchAndFuse(CorrectedSim3, mvpLoopMapPoints, mvpLoopMapLines);
...
void LoopClosing::MergeLocal()
{
    ...
    Verbose::PrintMess("MERGE: Initially there are " + to_string(pCurrentMap->KeyFramesInMap()) + " KFs and " + to_string(pCurrentMap->MapPointsInMap()) + " MPs in the active map " + messageLinesActiveMap, Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess("MERGE: Initially there are " + to_string(pMergeMap->KeyFramesInMap()) + " KFs and " + to_string(pMergeMap->MapPointsInMap()) + " MPs in the matched map " + messageLinesMatchedMap, Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("MERGE: Number of MPs in local window from active map: " + to_string(spLocalWindowMPs.size()) + messageLocalWindowLinesActiveMap, Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess("MERGE: Number of MPs in the active map: " + to_string(pCurrentMap->GetAllMapPoints().size()) + messageLinesActiveMap, Verbose::VERBOSITY_DEBUG);
    ...
    */    
    ...
    {
        Optimizer::LocalBundleAdjustment(mpCurrentKF, vpLocalCurrentWindowKFs, vpMergeConnectedKFs,&bStop,( mpTracker->IsLineTracking() ));
    }
...
void LoopClosing::MergeLocal2()
{
    ...
    // TODO: Luigi add map objects management
    SearchAndFuse(vpCurrentConnectedKFs, vpCheckFuseMapPoint, vpCheckFuseMapLine);
...
// TODO: Luigi add map objects management
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPointPtr> &vpMapPoints, vector<MapLinePtr> &vpMapLines)
{
...
// TODO: Luigi add map objects management
void LoopClosing::SearchAndFuse(const vector<KeyFramePtr> &vConectedKFs, vector<MapPointPtr> &vpMapPoints, vector<MapLinePtr> &vpMapLines)
{
    ORBmatcher matcher(0.8);
    std::shared_ptr<LineMatcher> pLineMatcher;    
    if(mpTracker->IsLineTracking())
