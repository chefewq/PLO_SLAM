...
void Map::AddMapObjects(MapObject* pMO)
{
    unique_lock<mutex> lock(mMutexMap);
...
void Map::SetReferenceMapObjects(const std::vector<MapObjectPtr> &vpMOs){
    unique_lock<mutex> lock(mMutexMap);
    mvpReferenceObjects= vpMOs;
}
...
MapObject* Map::GetObjectByTrackId(int id)
{
    unique_lock<mutex> lock(mMutexMap);
    for ( MapObject* pObj : mspMapObjects) {
        // 操作 pObj
        if(pObj->track_id==id)
        {
            return pObj;
...
void Map::GetSetOfReferenceMapObjects(std::set<MapObjectPtr>& setMOs){
    unique_lock<mutex> lock(mMutexMap);
    setMOs = std::set<MapObjectPtr>(mvpReferenceObjects.begin(),mvpReferenceObjects.end());
}
