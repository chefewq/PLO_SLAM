...
ImuCamPose::ImuCamPose(KeyFramePtr pKF):its(0)
{
    // Load IMU pose
...
ImuCamPose::ImuCamPose(Eigen::Matrix3d &_Rwc, Eigen::Vector3d &_twc, KeyFramePtr pKF): its(0)
{
...
Eigen::Vector3d ImuCamPose::ToBody(const Eigen::Vector3d &Xw) const
{
    return Rwb.transpose() * (Xw -twb);
}

Eigen::Vector3d ImuCamPose::ToCam(const Eigen::Vector3d &Xw, int cam_idx) const
{
    return Rcw[cam_idx] * Xw + tcw[cam_idx];
}
...
void ImuCamPose::Update(const double *pu)
{
    ...
    ut << pu[3], pu[4], pu[5];
#if 0
    ut = LeftJacobianSO3(pu[0], pu[1], pu[2])*ut; // se details below 
#endif 
    // Update body pose (right perturbation model)
    // Twb <-- Twb * exp(csi)   that is:
    // [Rwb,twb] <-- [Rwb,twb] * exp(csi) =~ [Rwb,twb] * [ExpSO3(ur), ut]
    // [  0,  1]     [  0,  1]                           [         0,  1]
    // Note: exp(csi)=[exp(phi), J(phi)*rho] = [exp(ur), J(ur)*ut] where (csi)=(phi,rho)=(ur,ut)  (see Barfoot's book)
    //                [     O^T,          1]   [    O^T,        1]
    //       and J(phi) is the left jacobian of SO(3) which can be approx to I if phi is small     
    //       By taking into account the left jacobian one should have twb += Rwb * J * ut;
    twb += Rwb * ut;
    Rwb = Rwb * ExpSO3(ur);
...
InvDepthPoint::InvDepthPoint(double _rho, double _u, double _v, KeyFramePtr pHostKF): u(_u), v(_v), rho(_rho),
...
void EdgeMono::linearizeOplus()
{
    ...
    _jacobianOplusXi = -proj_jac * Rcw; // _jacobianOplusXi = de/dPw = de/dPc * dPc/dPw  
                                        // where de/dPc=-proj_jac and dPc/dPw=Rcw (since Pc=Rcw*Pw+tcw)

    Eigen::Matrix<double,3,6> SE3deriv; // derivative with left perturbation model
    ...
    SE3deriv << 0.0,   z, -y,  1.0, 0.0, 0.0,  // [-[Xb]^, I]
                 -z, 0.0,  x,  0.0, 1.0, 0.0,
                  y,  -x, 0.0, 0.0, 0.0, 1.0;

    // TODO optimize this product
    _jacobianOplusXj = proj_jac * Rcb * SE3deriv;  // _jacobianOplusXj = de/d(mu) = de/dPc * dPc/dPb * dPb/d(mu)  
                                                   // Note that VPose uses a right perturbation model with is paramenter mu:
                                                   ...
                                                   //          de/dPc=-proj_jac, dPc/dPb=Rcb (since Pc=Rcb*Pb+tcw)
                                                   // Therefore:  _jacobianOplusXj = -proj_jac * Rcb * (-SE3deriv)
}
...
void EdgeMonoOnlyPose::linearizeOplus()
{
    ...
    //NOTE: [Luigi] for the derivation see the notes in EdgeMono::linearizeOplus()
    ...
    SE3deriv << 0.0, z,   -y, 1.0, 0.0, 0.0,
            -z , 0.0, x, 0.0, 1.0, 0.0,
            y ,  -x , 0.0, 0.0, 0.0, 1.0;
    _jacobianOplusXi = proj_jac * Rcb * SE3deriv; // sign different becasue of update mode
}
...
VertexVelocity::VertexVelocity(KeyFramePtr pKF)
{
...
VertexGyroBias::VertexGyroBias(KeyFramePtr pKF)
{
...
VertexAccBias::VertexAccBias(KeyFramePtr pKF)
{
...
Eigen::Matrix3d LeftJacobianSO3(const Eigen::Vector3d &v)
{
    return LeftJacobianSO3(v(0),v(1),v(2));
}

Eigen::Matrix3d LeftJacobianSO3(const double x, const double y, const double z)
{
    const double d2 = x*x+y*y+z*z;
    const double d = sqrt(d2);
    ...
    if(d<1e-5) {
        ...
        return Eigen::Matrix3d::Identity() + W*(1.0f-cos(d))/d2 + W*W*(d-sin(d))/(d2*d);
    }
}
