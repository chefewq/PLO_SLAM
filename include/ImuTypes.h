...
public:
    Eigen::Vector3f a;
    Eigen::Vector3f w;
    double t; // NOTE: [Luigi]: changed to double (timestamps should always be represented at least with double) 
...
public:
    IntegratedRotation(){}
    IntegratedRotation(const Eigen::Vector3f &angVel, const Bias &imuBias, const double &time);

public:
    double deltaT; //integration time  NOTE: [Luigi]: changed to double (timestamps should always be represented at least with double) 
...
public:
    ...
    void IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel, const double &dt);
    void Reintegrate();
...
public:
    double dT;  // NOTE: [Luigi]: changed to double (timestamps should always be represented at least with double) 
...
private:
    ...
    struct integrable
    {
        ...
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        integrable(){}
        integrable(const Eigen::Vector3f &a_, const Eigen::Vector3f &w_ , const double &t_):a(a_),w(w_),t(t_){}
        Eigen::Vector3f a, w;
        double t;  //NOTE: [Luigi]: changed to double (timestamps should always be represented at least with double) 
    };
...
Eigen::Matrix3f LeftJacobianSO3(const float &x, const float &y, const float &z);
Eigen::Matrix3f LeftJacobianSO3(const Eigen::Vector3f &v);
