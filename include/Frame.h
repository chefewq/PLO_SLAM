...
public:
    ...
    void EpiloarWithFundMatrix(const cv::Mat &ImGrayPre, const cv::Mat &ImGray);
    void CalcEpiDistChi2(const std::vector<cv::KeyPoint> &kp1, const std::vector<cv::Point2f> &kp2, cv::Mat &F, std::vector<uchar> &state);
    bool PointFilter(cv::Mat ImGray,std::vector<SegObject*> &dynamicArea,cv::Mat imDepth);
    void CheckObjectStatus(std::vector<double> epiErr,std::vector<SegObject*> &area);
    void RemoveMovingKeyPoints(const cv::Mat &ImGray,const cv::Mat &ImDepth,std::vector<SegObject*> &area);
    void RemoveMovingLine(const cv::Mat &ImGray,const cv::Mat &ImDepth,std::vector<SegObject*> &area);
...
public:
    ...
    int N_O;
    std::vector<float> mvObjectsDepth;

    std::vector<cv::KeyPoint> mvObjectsKeys;
    cv::Mat mvObjectsDescriptors;
    ...
    int mnDataset;
    cv::Mat dynaMask;
    vector<cv::Mat> mvDynamicArea;
    cv::Mat all_dynaMask;
    std::vector<cv::Rect2i> mvDynamicRect;
